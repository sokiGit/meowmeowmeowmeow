...
class MyWidget(QtWidgets.QWidget):
    ...
    def create_topbar(self):
        self.topbar = QtWidgets.QMenuBar(nativeMenuBar=True)

        #file_menu = topbar.addMenu("File")
        #file_menu.addAction("Hello, World!")

        self.layout.addWidget(self.topbar)

    def prompt_select_iface(self):
        sniff_notice = QtWidgets.QLabel(text="Select an interface by clicking a row in the table, then click the Select button.")
    ...
    def sniff_on_iface(self, iface : str, local_ip : str):
        ...
        def sniff_callback(data : dict):
            ...
            if socket_pair_id in socket_pair_cache:
                ...
                if data.get("ip") in ["192.81.241.191", "185.56.65.167", "185.56.65.171", "185.56.65.170", "185.56.65.169"]: return 
                ...
                def update_row_with_ip_info():
                    try:
                        ...
                        sniff_table.modifyItem(row_index, 3, f"{ip_api_data.get("country")}, {ip_api_data.get("regionName")}, {ip_api_data.get("city")}")
...
if __name__ == "__main__":
    ...
    widget = MyWidget()
    widget.request_change_title.connect(lambda txt: widget.setWindowTitle(f"MeowMeowMeowMeow{(" | "+txt) if txt or txt.strip() == "" else ""}"))
    widget.resize(800, 600)
    widget.show()

    widget.prompt_select_iface()
